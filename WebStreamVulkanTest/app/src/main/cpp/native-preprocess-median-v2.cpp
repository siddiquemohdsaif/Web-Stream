...
std::string webstreamRunMedianFilterV2Bytes(
    ...
    VulkanMeanFilter filter(input, mask, static_cast<uint32_t>(width),
                            ...
                            shader, false);
    LOGI("Vulkan median filter V2 setup complete");
    filter.warmUp();
    ...
    report << filter.deviceName() << "\n"
           ...
           << "threshold Y: " << std::clamp(thresholdY, 0, 255)
           << ", chroma: " << std::clamp(thresholdChroma, 0, 255) << "\n\n"
...
static jstring runNoiseFilter(
    ...
    try {
        ...
        std::vector<uint8_t> output;
        const std::string report = webstreamRunMedianFilterV2Bytes(
                input, mask, output, width, height, shader, thresholdY, thresholdChroma);
...
extern "C" JNIEXPORT jstring JNICALL
Java_app_builderx_ogfa_camerapipelinetest_LivePreprocessingActivity_nativeRunMedianFilterV2(
    JNIEnv* env, jobject object, jbyteArray inputArray, jbyteArray maskArray,
    jbyteArray outputArray, jint width, jint height, jbyteArray shaderArray,
    jint thresholdY, jint thresholdChroma) {
    return runNoiseFilter(env, object, inputArray, maskArray, outputArray,
...
extern "C" JNIEXPORT jstring JNICALL
Java_com_w3n_webstreamvulkantest_JoinCallActivity_nativeRunMedianFilterV2(
    JNIEnv* env, jobject object, jbyteArray inputArray, jbyteArray maskArray,
    jbyteArray outputArray, jint width, jint height, jbyteArray shaderArray,
    jint thresholdY, jint thresholdChroma) {
    return runNoiseFilter(env, object, inputArray, maskArray, outputArray,
