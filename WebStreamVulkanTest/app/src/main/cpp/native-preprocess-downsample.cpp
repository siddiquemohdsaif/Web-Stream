...
public:
    VulkanDownsample(const uint8_t* rgba, uint32_t rgbaStride,
        ...
        tileSrcW_ = mode == 3 ? 12u : (mode == 4 ? 8u : 4u);
        tileSrcH_ = mode == 3 ? 3u : (mode == 4 ? 2u : 1u);
        tileDstW_ = mode == 3 ? 8u : 4u;
        tileDstH_ = mode == 3 ? 2u : 1u;
...
private:
    ...
    uint32_t width_, height_, dstWidth_, dstHeight_;
...
std::string webstreamRunSobelBytes(
...
std::string webstreamRunMedianFilterV2Bytes(
    const std::vector<uint8_t>& input,
    ...
    const std::vector<uint8_t>& shader,
    jint thresholdY,
    jint thresholdChroma);

std::string webstreamRunDownsampleBytes(
    JNIEnv* env,
    jobject bitmap,
    std::vector<uint8_t>& output,
    jint width,
    ...
    double secondRunMs = 0.0;
    double totalMs = 0.0;
    try {
        VulkanDownsample downsample(static_cast<const uint8_t*>(bitmapPixels), bitmapInfo.stride,
                                    width, height, mode, shader);
        AndroidBitmap_unlockPixels(env, bitmap);
        bitmapPixels = nullptr;
        downsample.execute(1, false, false);
        secondRunMs = downsample.execute(1, true, false);
        totalMs = downsample.execute(static_cast<uint32_t>(benchmarkRuns), true, true);
        output = downsample.output();
        deviceName = downsample.deviceName();
        dstWidth = downsample.dstWidth();
        dstHeight = downsample.dstHeight();
    } catch (...) {
...
static jstring runDownsample(
    ...
    try {
        std::vector<uint8_t> output;
        const std::string report = webstreamRunDownsampleBytes(
                env, bitmap, output, width, height, mode,
                bytesFromJava(env, shaderArray), benchmarkRuns);
...
Java_com_w3n_webstreamvulkantest_JoinCallActivity_nativeRunPreprocessPipeline(
    JNIEnv* env, jclass, jobject bitmap, jbyteArray outputArray,
    jint width, jint height, jint mode, jbyteArray downsampleShaderArray,
    jbyteArray sobelShaderArray, jbyteArray medianShaderArray,
    jint thresholdY, jint thresholdChroma) {
    try {
        std::vector<uint8_t> downsampled;
        const std::string downsampleReport = webstreamRunDownsampleBytes(
                env, bitmap, downsampled, width, height, mode,
                bytesFromJava(env, downsampleShaderArray), 1);

        const int outWidth = mode == 3 ? width / 3 * 2 : (mode == 4 ? width / 2 : width);
        const int outHeight = mode == 3 ? height / 3 * 2 : (mode == 4 ? height / 2 : height);
        const size_t expected = static_cast<size_t>(outWidth) * outHeight * 3u;
        if (downsampled.size() != expected) {
            throw std::runtime_error("Downsample output has wrong size");
        }
        if (env->GetArrayLength(outputArray) != static_cast<jsize>(expected)) {
            throw std::runtime_error("Output array has wrong size");
        }

        std::vector<uint8_t> sobelMask;
        const std::string sobelReport = webstreamRunSobelBytes(
                downsampled, sobelMask, outWidth, outHeight,
                bytesFromJava(env, sobelShaderArray));

        std::vector<uint8_t> filtered;
        const std::string medianReport = webstreamRunMedianFilterV2Bytes(
                downsampled, sobelMask, filtered, outWidth, outHeight,
                bytesFromJava(env, medianShaderArray), thresholdY, thresholdChroma);
        if (filtered.size() != expected) {
            throw std::runtime_error("Filtered output has wrong size");
        }

        env->SetByteArrayRegion(outputArray, 0, static_cast<jsize>(filtered.size()),
                                reinterpret_cast<const jbyte*>(filtered.data()));

        std::ostringstream report;
        report << "Native preprocess pipeline complete\n\n"
               << "[downsample]\n" << downsampleReport << "\n\n"
               << "[sobel]\n" << sobelReport << "\n\n"
               << "[median_v2]\n" << medianReport;
...
Java_app_builderx_ogfa_camerapipelinetest_PreprocessingTest_nativeRunDownsample(
    JNIEnv* env, jclass clazz, jobject bitmap, jbyteArray outputArray,
    jint width, jint height, jint mode, jbyteArray shaderArray, jint benchmarkRuns) {
    return runDownsample(env, clazz, bitmap, outputArray, width, height, mode, shaderArray, benchmarkRuns);
}

extern "C" JNIEXPORT jstring JNICALL
Java_app_builderx_ogfa_camerapipelinetest_LivePreprocessingActivity_nativeRunDownsample(
    JNIEnv* env, jclass clazz, jobject bitmap, jbyteArray outputArray,
    jint width, jint height, jint mode, jbyteArray shaderArray, jint benchmarkRuns) {
    return runDownsample(env, clazz, bitmap, outputArray, width, height, mode, shaderArray, benchmarkRuns);
}
...
extern "C" JNIEXPORT jstring JNICALL
Java_com_w3n_webstreamvulkantest_JoinCallActivity_nativeRunDownsample(
    JNIEnv* env, jclass clazz, jobject bitmap, jbyteArray outputArray,
    jint width, jint height, jint mode, jbyteArray shaderArray, jint benchmarkRuns) {
    return runDownsample(env, clazz, bitmap, outputArray, width, height, mode, shaderArray, benchmarkRuns);
}
